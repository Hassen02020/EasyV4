 ...
 * - Mode Mock automatique si credentials MyGo absents
 */
...
/** Vérifie si le mode Mock est activé (credentials MyGo absents) */
function isMockMode(): boolean {
  const login = process.env.MYGO_LOGIN
  const password = process.env.MYGO_PASSWORD
  return !login || !password || login.length === 0 || password.length === 0
}
...
export class MyGoClient {
  ...
  listCities(): Promise<ListCityItemT[]> {
    if (isMockMode()) {
      return Promise.resolve(MOCK_CITIES)
    }
  ...
  listBoardings(): Promise<ListBoardingItemT[]> {
    if (isMockMode()) {
      return Promise.resolve([
        { Id: 1, Code: "RO", Name: "Logement seul", Description: "Sans pension" },
        { Id: 2, Code: "BB", Name: "Petit-déjeuner", Description: "Petit-déjeuner buffet" },
        { Id: 3, Code: "HB", Name: "Demi-pension", Description: "Petit-déjeuner + dîner" },
        { Id: 4, Code: "FB", Name: "Pension complète", Description: "Tous les repas inclus" },
        { Id: 5, Code: "AI", Name: "All Inclusive", Description: "Pension complète illimitée" },
      ])
    }
    return this.cachedCall(
      "ListBoarding",
  ...
  listHotels(cityId?: number): Promise<ListHotelItemT[]> {
    if (isMockMode()) {
      const hotels = cityId
        ? MOCK_HOTELS.filter((h) => h.City?.Id === cityId)
        : MOCK_HOTELS
  ...
  hotelDetail(hotelId: number): Promise<HotelDetailItemT | null> {
    if (isMockMode()) {
      return Promise.resolve(MOCK_HOTEL_DETAILS[hotelId] || null)
    }
    return this.cachedCall(
      "HotelDetail",
      { Hotel: hotelId },
  ...
  searchHotels(input: HotelSearchInput): Promise<{
    ...
    if (isMockMode()) {
      ...
      if (input.cityId) {
        results = results.filter((r) => r.Hotel.City?.Id === input.cityId)
      }
      // Filtrer par hôtel si spécifié
      if (input.hotelId) {
        results = results.filter((r) => r.Hotel.Id === input.hotelId)
      }
      // Filtrer par catégories (étoiles) si spécifié
      if (input.filters?.categories?.length) {
        results = results.filter((r) =>
          input.filters!.categories!.includes(r.Hotel.Category?.Id as number),
        )
      }
      // Filtrer par disponibilité
      if (input.filters?.onlyAvailable) {
        results = results.filter((r) =>
          r.Price.Boarding.some((b) =>
            b.Pax.some((p) =>
              p.Rooms.some((room) => !room.StopReservation && !room.OnRequest),
            ),
          ),
        )
      }
