/**
 * Inventaire mutable du Virtual World Hotel Supplier — en mémoire
 * (process-local), même principe que lib/vols/virtual-supplier/inventory-store.ts
 * (harness de test réaliste, pas une base de données fournisseur réelle).
 *
 * Disponibilité de BASE dérivée d'un hash déterministe (reproductible d'un
 * run à l'autre pour un même hôtel/dates), sur laquelle viennent s'appliquer
 ...
 * sous accès concurrents sur la MÊME chambre/hôtel, un seul appelant peut
 * faire passer la disponibilité sous zéro.
 */
...
/** Réserve `quantity` chambres sur l'offre, atomiquement. */
