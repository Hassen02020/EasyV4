  ...
  return (
    ...
    >
      <h2 className="text-primary mb-5 text-lg font-bold">
        {t("filter_sidebar.title")}
      </h2>

      <div className="space-y-4">
        {/* Tarifs et disponibilités */}
        <FilterSection title={t("filter_sidebar.pricing")}>
          <CheckboxItem
            id="recommended"
            label={t("filter_sidebar.recommended")}
          ...
          <CheckboxItem
            id="available"
            label={t("filter_sidebar.available_only")}
          ...
          <CheckboxItem
            id="free-cancel"
            label={t("filter_sidebar.free_cancellation")}
        ...
        {(facets?.stars.length ?? 0) > 0 && (
          <FilterSection title={t("filter_sidebar.category")}>
        ...
        {/* Prix par nuit */}
        {priceMax > priceMin && (
          <FilterSection title={t("filter_sidebar.price_per_night").replace("{currency}", currency)}>
            <div className="px-1">
              <Slider
              ...
              <div className="text-muted-foreground mt-2 flex justify-between text-sm">
                <span>
                  {currentRange[0].toLocaleString(language === "fr" ? "fr-FR" : language === "ar" ? "ar-TN" : "en-US")} {currency}
                </span>
                <span>
                  {currentRange[1].toLocaleString(language === "fr" ? "fr-FR" : language === "ar" ? "ar-TN" : "en-US")} {currency}
                </span>
              </div>
            </div>
          </FilterSection>
        ...
        {(facets?.boardings.length ?? 0) > 0 && (
          <FilterSection title={t("filter_sidebar.meal_plan")}>
        ...
        {(facets?.facilities.length ?? 0) > 0 && (
          <FilterSection title={t("filter_sidebar.facilities")} defaultOpen={false}>
