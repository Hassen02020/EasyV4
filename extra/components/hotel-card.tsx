...
function formatMoney(amount: number, currency = "TND", locale = "fr-FR"): string {
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="bg-muted group relative h-48 w-full shrink-0 md:h-auto md:w-64 lg:w-72">
          ...
          <button
            ...
            aria-label={t("hotel_card.view_image").replace("{number}", String(currentImage))}
          >
          ...
          <button
            ...
            aria-label={t("hotel_card.view_image").replace("{number}", String(currentImage + 2))}
          >
          ...
          <div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-1">
            {hotel.images.map((_, i) => (
              <button
                ...
                aria-label={t("hotel_card.view_image").replace("{number}", String(i + 1))}
              />
            ))}
          </div>
          ...
          <button
            ...
            aria-label={
              isWishlisted ? t("hotel_card.remove_wishlist") : t("hotel_card.add_wishlist")
            }
          >
            <Heart
              className={`h-4 w-4 ${
          ...
          <div className="border-border flex min-w-[150px] flex-col items-end justify-between border-t pt-4 md:border-t-0 md:border-l md:pt-0 md:pl-4">
            ...
            {hotel.discountPercent > 0 && (
              <div className="mb-2 rounded bg-gradient-to-r from-amber-500 to-orange-500 px-2 py-1 text-xs font-semibold text-white">
                {t("hotel_card.exclusive")}
              </div>
            )}

            <div className="text-right">
              <p className="text-muted-foreground mb-1 text-xs">{t("hotel_card.from")}</p>
              <div className="flex items-baseline justify-end gap-1">
                <span className="text-primary text-2xl font-bold">
                  {formatMoney(hotel.discountedPrice, currency, locale)}
                </span>
              </div>
              ...
              >
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <span className="rounded bg-emerald-100 px-2 py-0.5 text-xs text-emerald-700">
                        {t("hotel_card.free_cancellation_before").replace("{date}", room.freeCancellationDate)}
                      </span>
                      <span
                      ...
                      >
                        {room.available ? t("hotel_card.available") : t("hotel_card.on_request")}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-primary text-lg font-bold">
                    {room.price.toLocaleString(locale)}
                  </span>
