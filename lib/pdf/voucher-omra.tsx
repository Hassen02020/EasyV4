 ...
 *
 * Même structure que `voucher-hotel.tsx` (mêmes styles, même moteur), adaptée
 * aux données d'un séjour Omra confirmé (groupe de pèlerins, dates de vol
 * aller/retour) au lieu d'un séjour hôtel classique.
 */
...
function OmraVoucherDocument({ data }: { data: OmraVoucherData }) {
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.subtitle}>Voucher de Confirmation Omra</Text>
          </View>
        ...
        <Text style={styles.sectionTitle}>Contact du groupe</Text>
        ...
        <Text style={styles.sectionTitle}>Programme Omra</Text>
        <View style={styles.table}>
          <View style={styles.row}>
          ...
          <View style={styles.rowLast}>
            <Text style={styles.cellLabel}>Pèlerins</Text>
            <Text style={styles.cellValue}>
              {data.pilgrimsCount} pèlerin{data.pilgrimsCount > 1 ? "s" : ""}
            </Text>
          </View>
        </View>
...
export async function renderOmraVoucherPdf(data: OmraVoucherData): Promise<Uint8Array> {
  const buffer = await renderToBuffer(<OmraVoucherDocument data={data} />)
