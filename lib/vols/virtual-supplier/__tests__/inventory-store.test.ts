/**
 * Concurrence de l'inventaire vols — même risque que
 * lib/mygo/virtual-supplier/__tests__/inventory-store.test.ts : un
...
test("SÉCURITÉ — CONCURRENCE : dernier siège (inventaire=1), deux réservations simultanées => une seule réussit", async () => {
  resetInventory()
  ...
  assert.equal(successes, 1, "exactement un gagnant sur le dernier siège")
  assert.equal(currentAvailability(KEY), 0)
})

test("SÉCURITÉ — CONCURRENCE : 10 tentatives simultanées sur 1 siège => exactement 1 succès, 9 échecs, jamais négatif", async () => {
  resetInventory()
  let avail = currentAvailability(KEY)
  while (avail > 1) {
    await reserve(KEY, 1)
