 ...
 * (`getPackageWithDepartures`) : `withPublicAgencyContext` (lecture scopée à
 * une agence, aucune session requise, jamais `is_super_admin`) + `agencyId`
 * résolu via `getDefaultAgencyId()` — jamais une agence arbitraire, jamais
 * les données d'une autre agence exposées (une seule agence de vente directe
 * existe pour le B2C, voir lib/agencies/default-agency.ts). Réutilise
